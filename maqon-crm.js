...
function next(id){let d=normalize(load()),l=d.leads.find(x=>x.id===id);if(!l)return;if(l.status==='Proposta'){location.href='maqon-negociacao.html?lead='+encodeURIComponent(id);return}let i=STAGES.indexOf(l.status);if(i<STAGES.length-1)move(id,STAGES[i+1])}
