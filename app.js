...
function viewProposal(i){
 ...
 if(maker.toLowerCase()==='sany' && model.toLowerCase().includes('stc250t5')) specs=[['CAPACIDADE DE CARGA','25 t'],['POTÊNCIA','320 hp'],['ALTURA DA LANÇA','47 m']];
 const photo=`maqon-${visual}.jpg`;
 const scope=p.scope||`Aquisição do ${maker} ${model}, ${category.toLowerCase()}, conforme especificações técnicas do fabricante.`;
 ...
 *{box-sizing:border-box}html,body{margin:0;background:#061015;font-family:Arial,Helvetica,sans-serif;color:#fff}.toolbar{height:56px;background:#071218;display:flex;align-items:center;justify-content:space-between;padding:0 22px;position:sticky;top:0;z-index:30;border-bottom:1px solid #d9aa00}.toolbar button{border:0;border-radius:5px;padding:10px 16px;font-weight:800;cursor:pointer;margin-left:8px}.gold{background:#f4c400}.dark{background:#263740;color:#fff}.stage{padding:18px;display:flex;justify-content:center}.sheet{position:relative;width:min(1024px,100%);aspect-ratio:2/3;background:#071015 url('maqon-proposta-modelo.png') top center/100% 100% no-repeat;box-shadow:0 12px 45px #000;overflow:hidden}.dyn{position:absolute;z-index:5}.cover{background:linear-gradient(135deg,#07151b,#061014)}.heroPhoto{left:41.5%;top:0;width:37.5%;height:20.1%;background:url('${photo}') center/cover no-repeat;z-index:2}.heroPhoto:after{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(6,16,20,.55),transparent 25%,transparent 80%,rgba(6,16,20,.2))}.heroLine{left:79.2%;top:0;width:2.2%;height:20.3%;background:linear-gradient(75deg,transparent 25%,#f2c300 27%,#f2c300 42%,#111 44%,#111 58%,#f2c300 60%,#f2c300 72%,transparent 74%);z-index:6}.proposalNum{right:6.7%;top:5.2%;font-size:17px}.client{left:10.4%;top:25.1%;width:37%;height:2.7%;padding:3px 0 0 0;font-size:18px;font-weight:700}.company{left:57.9%;top:25.1%;width:33%;height:2.7%;font-size:18px;font-weight:700}.mode{left:10.4%;top:29.6%;font-size:17px;font-weight:700}.validity{left:46.2%;top:29.6%;font-size:17px;font-weight:700}.issue{left:76.5%;top:29.6%;font-size:17px;font-weight:700}.eqPhoto{left:3.7%;top:35.25%;width:44.8%;height:17.1%;background:url('${photo}') center/cover no-repeat;border-radius:9px;border:1px solid #9c7900}.eqPanel{left:49.7%;top:35.25%;width:46.8%;height:17.1%;padding:12px 17px;background:linear-gradient(135deg,rgba(6,18,24,.98),rgba(5,14,19,.98));border:1px solid #49606a;border-radius:7px}.eqPanel small{color:#aebec5;font-size:14px}.eqPanel h2{font-size:21px;margin:7px 0 12px}.eqPanel p{font-size:14px;line-height:1.45;margin:0 0 13px;border-bottom:1px solid #33454d;padding-bottom:13px}.specs{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.spec{padding:4px 10px;border-right:1px solid #41535b}.spec:last-child{border:0}.spec label{display:block;color:#aebec5;font-size:12px;margin-bottom:5px}.spec b{font-size:17px}.price{left:11.9%;top:58.15%;width:35.5%;height:6.7%;background:linear-gradient(120deg,#3a2b05,#090f12 80%);padding:4px 8px}.price b{display:block;color:#ffc400;font-size:43px;line-height:1.05}.price span{font-size:17px}.terms{left:58.6%;top:56.8%;width:35%;height:2.8%;background:#07151b;padding:4px 0;font-size:15px;font-weight:700}.delivery{left:58.6%;top:60.5%;width:35%;height:2.8%;background:#07151b;padding:4px 0;font-size:15px;font-weight:700}.warranty{left:58.6%;top:64.2%;width:35%;height:2.8%;background:#07151b;padding:4px 0;font-size:15px;font-weight:700}.scopeCover{left:8.5%;top:70.45%;width:82%;height:8.7%;background:#07151b;padding:4px 0 0 8px;font-size:14px;line-height:1.68}.scopeCover div:before{content:'✓';color:#ffc400;font-weight:900;margin-right:10px}.obsCover{left:7.5%;top:82.4%;width:47%;height:4.6%;background:#061014;padding:6px 0;font-size:13px;line-height:1.45}.printNote{display:none}@media(max-width:760px){.toolbar{font-size:12px}.stage{padding:4px}.sheet{width:100%}}@media print{.toolbar{display:none}.stage{padding:0}.sheet{width:210mm;height:315mm;box-shadow:none}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@page{size:A4;margin:0}}
 </style></head><body><header class="toolbar"><b>Prévia oficial MAQON — ${esc(num)}</b><div><button class="dark" onclick="window.close()">Fechar</button><button class="gold" onclick="window.print()">Imprimir / Salvar PDF</button></div></header><main class="stage"><article class="sheet">
 <div class="heroPhoto dyn"></div><div class="heroLine dyn"></div><div class="proposalNum dyn">${esc(num)}</div>
 <div class="client dyn cover">${esc(p.client)}</div><div class="company dyn cover">${esc(p.company||'—')}</div><div class="mode dyn cover">${esc(p.mode)}</div><div class="validity dyn cover">${esc(validity)}</div><div class="issue dyn cover">${esc(issue)}</div>
 <div class="eqPhoto dyn"></div><div class="eqPanel dyn"><small>EQUIPAMENTO SELECIONADO</small><h2>${esc(maker)} ${esc(model)} — ${esc(category)}</h2><p>${esc(desc)}</p><div class="specs">${specs.map(s=>`<div class="spec"><label>${esc(s[0])}</label><b>${esc(s[1])}</b></div>`).join('')}</div></div>
 <div class="price dyn"><b>${brl(p.value)}</b><span>Investimento conforme proposta comercial</span></div><div class="terms dyn">${esc(p.terms||'Conforme negociação')}</div><div class="delivery dyn">A combinar</div><div class="warranty dyn">Conforme fabricante</div>
 <div class="scopeCover dyn"><div>${esc(scope)}</div><div>Suporte na análise técnica e comparativa com outras opções de mercado.</div><div>Orientação sobre manutenção preventiva e custo operacional.</div><div>Acompanhamento no processo de aquisição e entrega do equipamento.</div><div>Treinamento operacional básico (conforme fabricante).</div></div>
 <div class="obsCover dyn">Esta proposta é válida pelo período informado e pode ser ajustada conforme negociação. Valores sujeitos à alteração sem aviso prévio.</div>
