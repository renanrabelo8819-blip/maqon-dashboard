 ...
 *{box-sizing:border-box}html,body{margin:0;background:#071015;color:#fff;font-family:Arial,Helvetica,sans-serif}.toolbar{position:sticky;top:0;z-index:20;height:58px;background:#0b1820;border-bottom:1px solid #2c414b;display:flex;align-items:center;justify-content:space-between;padding:0 22px;box-shadow:0 4px 18px #0008}.toolbar h1{font-size:18px;margin:0}.toolbar .btns{display:flex;gap:9px}.toolbar button{border:1px solid #465963;background:#13232c;color:#fff;border-radius:5px;padding:10px 17px;font-weight:700;cursor:pointer}.toolbar .gold{background:#f4c400;color:#111;border-color:#f4c400}.stage{min-height:calc(100vh - 58px);padding:14px;display:flex;justify-content:center;background:radial-gradient(circle at 50% 20%,#152731,#071015 60%)}.sheet{position:relative;width:min(780px,100%);aspect-ratio:1024/1536;background:#050b0e url('maqon-proposta-modelo.png') center top/100% 100% no-repeat;box-shadow:0 12px 45px #000;border:1px solid #263b44}.data{position:absolute;color:#fff;font-weight:800;text-shadow:0 1px 2px #000;background:#071015e8;border-radius:3px;padding:2px 5px;line-height:1.12}.client{left:10.4%;top:25.6%;font-size:1.25vw;max-font-size:15px}.company{left:55.5%;top:25.6%;font-size:1.25vw}.mode{left:10.4%;top:30.2%;font-size:1.15vw}.valid{left:43.5%;top:30.2%;font-size:1.15vw}.issue{left:73.8%;top:30.2%;font-size:1.15vw}.eq{left:52.8%;top:39.0%;font-size:1.35vw;color:#f4c400;max-width:40%}.value{left:14.1%;top:59.2%;font-size:2.35vw;color:#f4c400}.scope{left:9.4%;top:72.6%;width:80%;font-size:.94vw;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.ref{right:4.8%;top:5.6%;font-size:1.15vw;background:transparent}.note{position:absolute;left:0;right:0;bottom:-34px;text-align:center;color:#93a6ae;font-size:11px}.mobile-data{display:none}@media(max-width:850px){.data{font-size:clamp(7px,1.55vw,12px)}.value{font-size:clamp(13px,3vw,24px)}.eq{font-size:clamp(8px,1.7vw,13px)}.scope{font-size:clamp(6px,1.15vw,9px)}}@media print{.toolbar{display:none}.stage{padding:0;background:#fff}.sheet{width:100%;height:auto;box-shadow:none;border:0}.note{display:none}@page{size:A4;margin:0}}
 </style></head><body><header class="toolbar"><h1>Proposta Comercial</h1><div class="btns"><button onclick="window.print()">▣ Imprimir / Salvar PDF</button><button class="gold" onclick="window.close()">✕ Fechar</button></div></header><main class="stage"><article class="sheet" aria-label="Proposta comercial MAQON"><span class="data ref">${esc(num)}</span><span class="data client">${esc(p.client)}</span><span class="data company">${esc(p.company||'—')}</span><span class="data mode">${esc(p.mode)}</span><span class="data valid">${esc(valid)}</span><span class="data issue">${esc(issue)}</span><span class="data eq">${esc(p.equipment)}</span><span class="data value">${brl(p.value)}</span><span class="data scope">${esc(p.scope||'')}</span><div class="note">Visual MAQON fiel à arte aprovada • dados da proposta sobrepostos automaticamente</div></article></main></body></html>`;
