...
function viewProposal(i){const p=proposals[i];if(!p)return;const t=`MAQON — PROPOSTA COMERCIAL\n${proposalNumber(i)}\n\nCliente: ${p.client}\nEmpresa: ${p.company||'—'}\nEquipamento: ${p.equipment}\nModalidade: ${p.mode}\nValor: ${brl(p.value)}\nValidade: ${p.validity||'—'}\nStatus: ${p.status}\n\nEscopo:\n${p.scope||'—'}\n\nCondições comerciais:\n${p.terms||'—'}\n\nMAQON — Consultoria em Equipamentos Pesados`;const w=window.open('','_blank');if(!w)return alert(t);w.document.write(`<html><head><title>${proposalNumber(i)} - MAQON</title><style>body{font-family:Arial;max-width:850px;margin:40px auto;padding:30px}h1{color:#b88700}pre{white-space:pre-wrap;font:16px/1.55 Arial;border-top:3px solid #d6a800;padding-top:24px}button{padding:10px 18px}</style></head><body><h1>MAQON</h1><pre>${esc(t)}</pre><button onclick="window.print()">Imprimir / Salvar em PDF</button></body></html>`);w.document.close()}
