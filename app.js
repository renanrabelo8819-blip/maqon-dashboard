 ...
 *{box-sizing:border-box}html,body{margin:0;background:#071015;color:#fff;font-family:Arial,Helvetica,sans-serif}
 .toolbar{position:sticky;top:0;z-index:20;height:58px;background:#0b1820;border-bottom:1px solid #2c414b;display:flex;align-items:center;justify-content:space-between;padding:0 22px;box-shadow:0 4px 18px #0008}
 .toolbar h1{font-size:18px;margin:0}.toolbar .btns{display:flex;gap:9px}
 .toolbar button{border:1px solid #465963;background:#13232c;color:#fff;border-radius:5px;padding:10px 17px;font-weight:700;cursor:pointer}
 .toolbar .gold{background:#f4c400;color:#111;border-color:#f4c400}
 .stage{min-height:calc(100vh - 58px);padding:14px;display:flex;justify-content:center;background:radial-gradient(circle at 50% 20%,#152731,#071015 60%)}
 .sheet{width:min(780px,100%);aspect-ratio:1024/1536;background:#050b0e url('maqon-proposta-modelo.png') center top/100% 100% no-repeat;box-shadow:0 12px 45px #000;border:1px solid #263b44}
 @media print{.toolbar{display:none}.stage{padding:0;background:#fff}.sheet{width:100%;box-shadow:none;border:0}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@page{size:A4;margin:0}}
 </style></head><body>
 <header class="toolbar"><h1>Proposta Comercial — ${esc(num)}</h1><div class="btns"><button onclick="window.print()">▣ Imprimir / Salvar PDF</button><button class="gold" onclick="window.close()">✕ Fechar</button></div></header>
 <main class="stage"><article class="sheet" aria-label="Proposta comercial MAQON"></article></main>
 </body></html>`;
