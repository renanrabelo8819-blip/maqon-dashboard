...
function viewProposal(i){
 ...
 const isApprovedKomatsu=(maker.toLowerCase()==='komatsu' && model.toLowerCase()==='pc210');
 ...
 const label=key.includes('retro')?'RETROESCAVADEIRA':key.includes('motonivel')?'MOTONIVELADORA':key.includes('carregadeira')?'PÁ CARREGADEIRA':key.includes('basculante')?'CAMINHÃO BASCULANTE':key.includes('pipa')?'CAMINHÃO PIPA':key.includes('guindaste')?'GUINDASTE':key.includes('munck')?'CAMINHÃO MUNCK':key.includes('trator')?'TRATOR DE ESTEIRAS':'ESCAVADEIRA';
 const visualMap={
  'escavadeira':'0% 13%','retroescavadeira':'25% 13%','motoniveladora':'50% 13%','pa carregadeira':'75% 13%','trator de esteiras':'100% 13%',
  'guindaste':'0% 72%','munck':'25% 72%','caminhao basculante':'50% 72%','caminhao pipa':'75% 72%','outro':'100% 72%'
 };
 const visualKey=key.includes('retro')?'retroescavadeira':key.includes('motonivel')?'motoniveladora':key.includes('carregadeira')?'pa carregadeira':key.includes('basculante')?'caminhao basculante':key.includes('pipa')?'caminhao pipa':key.includes('guindaste')?'guindaste':key.includes('munck')?'munck':key.includes('trator')?'trator de esteiras':key.includes('escav')?'escavadeira':'outro';
 const visualPos=visualMap[visualKey]||visualMap.outro;
 const desc=key.includes('basculante')?'Transporte e movimentação de materiais para obras, mineração e terraplenagem.':key.includes('pipa')?'Abastecimento, umectação de vias e apoio operacional em obras.':key.includes('guindaste')||key.includes('munck')?'Içamento, movimentação e posicionamento seguro de cargas.':key.includes('motonivel')?'Nivelamento, acabamento e conformação de vias e terrenos.':key.includes('retro')?'Escavação, carregamento e apoio versátil a obras.':key.includes('carregadeira')?'Carregamento e movimentação de materiais com alta produtividade.':'Escavação, terraplenagem e aplicações de alta produtividade.';
 const w=window.open('','_blank');if(!w)return alert('Autorize pop-ups para visualizar a proposta.');
 if(isApprovedKomatsu){
   const html=`<!doctype html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(num)} - MAQON</title><style>*{box-sizing:border-box}html,body{margin:0;background:#071015;color:#fff;font-family:Arial}.toolbar{height:58px;background:#0b1820;display:flex;align-items:center;justify-content:space-between;padding:0 22px}.toolbar button{border:0;border-radius:5px;padding:10px 16px;font-weight:800;cursor:pointer;margin-left:8px}.gold{background:#f4c400}.dark{background:#263740;color:#fff}.stage{padding:14px;display:flex;justify-content:center}.sheet{width:min(780px,100%);aspect-ratio:1024/1536;background:url('maqon-proposta-modelo.png') center top/100% 100% no-repeat;box-shadow:0 12px 45px #000}@media print{.toolbar{display:none}.stage{padding:0}.sheet{width:100%;box-shadow:none}@page{size:A4;margin:0}}</style></head><body><header class="toolbar"><b>Proposta Comercial — ${esc(num)}</b><div><button class="dark" onclick="window.close()">Fechar</button><button class="gold" onclick="window.print()">Imprimir / Salvar PDF</button></div></header><main class="stage"><article class="sheet"></article></main></body></html>`;
 ...
 *{box-sizing:border-box}body{margin:0;background:#061015;color:#fff;font-family:Arial,Helvetica,sans-serif}.toolbar{height:58px;background:#0b1820;display:flex;align-items:center;justify-content:space-between;padding:0 22px}.toolbar button{border:0;border-radius:5px;padding:10px 16px;font-weight:800;cursor:pointer;margin-left:8px}.gold{background:#f4c400}.dark{background:#263740;color:#fff}.page{width:min(790px,calc(100% - 24px));margin:14px auto;background:#071015;border:1px solid #344952;box-shadow:0 15px 55px #000}.hero{padding:32px;background:linear-gradient(125deg,#071015,#15170f);border-bottom:3px solid #e4b900;display:flex;justify-content:space-between}.brand{font-size:48px;font-weight:900}.brand b{color:#f4c400}.hero h1{margin:0;text-align:right}.hero h1 b{display:block;color:#f4c400}.content{padding:25px 32px}.title{display:flex;align-items:center;gap:10px;color:#f4c400;font-weight:900;margin:18px 0 10px}.title:after{content:"";height:2px;background:#d7ac00;flex:1}.grid{display:grid;grid-template-columns:1fr 1fr;gap:9px}.three{grid-template-columns:repeat(3,1fr)}.card{background:#0b181e;border:1px solid #334852;border-radius:7px;padding:13px}.card small{display:block;color:#9aadb5;font-size:10px;text-transform:uppercase;margin-bottom:5px}.equipment{display:grid;grid-template-columns:42% 58%;gap:10px}.machine{min-height:245px;border:1px solid #d5aa00;border-radius:7px;background:#071015 center/cover no-repeat;position:relative;overflow:hidden;display:flex;align-items:flex-end;justify-content:center;text-align:center;padding:18px}.machine:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,transparent 42%,rgba(3,9,12,.94) 88%)}.machine .caption{position:relative;z-index:2}.machine .type{font-size:21px;color:#f4c400;font-weight:900;text-shadow:0 2px 4px #000}.machine .name{font-size:15px;font-weight:800;margin-top:5px;text-shadow:0 2px 4px #000}.eq{background:#0b181e;border:1px solid #334852;border-radius:7px;padding:17px}.eq h2{color:#f4c400}.specs{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-top:18px}.price{border:1px solid #e4b900;border-left:9px solid #f4c400;border-radius:7px;padding:20px;background:linear-gradient(120deg,#4a3809,#0b1214 70%)}.price strong{display:block;color:#f4c400;font-size:34px;margin:7px 0}.scope{background:#0b181e;border:1px solid #334852;border-radius:7px;padding:15px;line-height:1.55}.footer{border-top:3px solid #e4b900;padding:17px 32px;display:flex;justify-content:space-between;font-size:10px}.footer b{color:#f4c400}@media(max-width:650px){.grid,.three,.equipment{grid-template-columns:1fr}}@media print{.toolbar{display:none}.page{width:100%;margin:0;box-shadow:none}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@page{size:A4;margin:6mm}}
 </style></head><body><header class="toolbar"><b>Proposta Comercial — ${esc(num)}</b><div><button class="dark" onclick="window.close()">Fechar</button><button class="gold" onclick="window.print()">Imprimir / Salvar PDF</button></div></header><article class="page"><header class="hero"><div><div class="brand"><b>M</b>AQON</div><b>CONSULTORIA EM EQUIPAMENTOS PESADOS</b><p>PLANEJAMENTO • DADOS • ESTRATÉGIA • RESULTADOS</p></div><h1>PROPOSTA <b>COMERCIAL</b><small>${esc(num)}</small></h1></header><main class="content"><div class="title">CLIENTE E PROPOSTA</div><div class="grid"><div class="card"><small>Cliente</small><b>${esc(p.client)}</b></div><div class="card"><small>Empresa</small><b>${esc(p.company||'—')}</b></div></div><div class="grid three" style="margin-top:9px"><div class="card"><small>Modalidade</small><b>${esc(p.mode)}</b></div><div class="card"><small>Validade</small><b>${esc(validity)}</b></div><div class="card"><small>Emissão</small><b>${esc(issue)}</b></div></div><div class="title">EQUIPAMENTO</div><div class="equipment"><div class="machine" style="background-image:url('maqon-biblioteca-equipamentos.png');background-position:${visualPos}"><div class="caption"><div class="type">${esc(label)}</div><div class="name">${esc(maker)} ${esc(model)}</div></div></div><div class="eq"><small>EQUIPAMENTO SELECIONADO</small><h2>${esc(maker)} ${esc(model)} — ${esc(category)}</h2><p>${esc(desc)}</p><div class="specs"><div class="card"><small>Ano</small><b>${esc(year)}</b></div><div class="card"><small>Horímetro</small><b>${esc(hours)} h</b></div><div class="card"><small>Disponibilidade</small><b>${esc(availability)}%</b></div></div></div></div><div class="title">INVESTIMENTO</div><div class="price"><small>VALOR TOTAL</small><strong>${brl(p.value)}</strong><span>${esc(p.terms||'Conforme negociação')}</span></div><div class="title">ESCOPO DA PROPOSTA</div><div class="scope">${esc(p.scope||`Solução para ${maker} ${model} — ${category}, conforme especificações cadastradas na plataforma MAQON.`)}</div><div class="title">OBSERVAÇÕES</div><p>Esta proposta é válida pelo período informado e pode ser ajustada conforme negociação. Valores e condições sujeitos à validação final.</p></main><footer class="footer"><span><b>MAQON</b> • Consultoria em Equipamentos Pesados</span><span>MÁQUINAS CERTAS, RESULTADOS MAIORES.</span></footer></article></body></html>`;
