...
const proposalEquipmentAuto=document.querySelector('#proposalEquipment');
if(proposalEquipmentAuto)proposalEquipmentAuto.addEventListener('change',()=>{
 const eq=equipments[Number(proposalEquipmentAuto.value)];
 const v=document.querySelector('#proposalValue');
...
function maqonProposalData(eq){
 const category=(eq?.[0]||'Equipamento').trim(),maker=(eq?.[1]||'').trim(),model=(eq?.[2]||'').trim();
 const norm=s=>String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
 const id=norm(maker+' '+model),cat=norm(category);
 let photo='maqon-outro.jpg',desc='Equipamento selecionado conforme especificações cadastradas na plataforma MAQON.';
 let specs=[['ANO',eq?.[3]||'—'],['HORÍMETRO',(eq?.[4]||'—')+' h'],['DISPONIBILIDADE',(eq?.[8]||'—')+'%']];
 if(id.includes('sany')&&id.includes('stc250t5')){photo='maqon-guindaste.jpg';desc='Guindaste rodoviário para içamento, movimentação e posicionamento seguro de cargas.';specs=[['CAPACIDADE DE CARGA','250 t'],['POTÊNCIA','320 hp'],['ALTURA DA LANÇA','47 m']]}
 ...
 else if(id.includes('caterpillar')&&id.includes('966l')){photo='maqon-pa-carregadeira.jpg';desc='Pá carregadeira para carregamento e movimentação de materiais com alta produtividade.';specs=[['POTÊNCIA','308 hp'],['PESO OPERACIONAL','24.000 kg'],['CAÇAMBA','4,2 m³']]}
 ...
 else if(cat.includes('retro')){photo='maqon-retroescavadeira.jpg';desc='Retroescavadeira para escavação, carregamento e apoio em obras.'}
 else if(cat.includes('motonivel')){photo='maqon-motoniveladora.jpg';desc='Motoniveladora para nivelamento, acabamento e conformação de vias e terrenos.'}
 else if(cat.includes('carregadeira')){photo='maqon-pa-carregadeira.jpg';desc='Pá carregadeira para carregamento e movimentação de materiais.'}
 else if(cat.includes('trator')){photo='maqon-trator-esteiras.jpg';desc='Trator de esteiras para terraplenagem, corte e preparação de terrenos.'}
 else if(cat.includes('guindaste')){photo='maqon-guindaste.jpg';desc='Guindaste para içamento e movimentação segura de cargas.'}
 else if(cat.includes('munck')){photo='maqon-munck.jpg';desc='Caminhão munck para transporte, içamento e movimentação de cargas.'}
 else if(cat.includes('basculante')){photo='maqon-caminhao-basculante.jpg';desc='Caminhão basculante para transporte e descarga de materiais.'}
 else if(cat.includes('pipa')){photo='maqon-caminhao-pipa.jpg';desc='Caminhão-pipa para abastecimento, umectação de vias e apoio operacional.'}
 else if(cat.includes('escav')){photo='maqon-escavadeira.jpg';desc='Escavadeira para escavação, carregamento e terraplenagem.'}
 return {category,maker,model,photo,desc,specs};
}
function viewProposal(i){
 const p=proposals[i];if(!p)return;
 const num=proposalNumber(i),eq=equipments.find(x=>equipmentName(x)===p.equipment)||null,d=maqonProposalData(eq);
 ...
 const scope=p.scope||'Aquisição para operação.';
 ...
 *{box-sizing:border-box}html,body{margin:0;background:#061016;color:#fff;font-family:Arial,Helvetica,sans-serif}.toolbar{height:50px;background:#0b1820;display:flex;justify-content:space-between;align-items:center;padding:0 18px;position:sticky;top:0;z-index:20}.toolbar button{border:0;border-radius:5px;padding:9px 13px;font-weight:800;cursor:pointer;margin-left:8px}.goldbtn{background:#ffd000}.darkbtn{background:#263740;color:#fff}.stage{display:flex;justify-content:center;padding:14px}.sheet{width:794px;max-width:100%;background:#020b10;border:1px solid #263b44;box-shadow:0 12px 45px #000;overflow:hidden}.hero{height:252px;display:grid;grid-template-columns:1fr 190px;background:url('maqon-identidade-referencia.png') left center/cover no-repeat;border-bottom:3px solid #e5bb00;position:relative}.hero:after{content:"";position:absolute;left:0;right:190px;top:0;bottom:0;background:linear-gradient(90deg,rgba(0,0,0,.05),rgba(0,0,0,.08));pointer-events:none}.proposal{z-index:2;background:rgba(2,8,11,.96);border-left:3px solid #e7ba00;padding:22px 16px}.proposal h1{font-size:20px;margin:0;line-height:1.05}.proposal h1 b{display:block;color:#ffd000;font-size:26px}.proposal .num{font-size:14px;font-weight:900;margin:8px 0 20px}.benefit{font-size:10px;line-height:1.45;margin:9px 0}.content{padding:0 25px 15px}.section{display:flex;align-items:center;gap:12px;color:#ffd000;font-size:16px;font-weight:900;margin:10px 0}.section:after{content:"";height:2px;background:#e5bb00;flex:1}.grid2,.grid3{display:grid;gap:8px}.grid2{grid-template-columns:1fr 1fr}.grid3{grid-template-columns:1fr 1fr 1fr}.card,.eqinfo,.scope{background:#06171f;border:1px solid #35525e;border-radius:7px}.card{padding:10px 14px;min-height:58px}.card small,.eqinfo small,.spec small{display:block;color:#bdc9cd;font-size:9px;text-transform:uppercase;margin-bottom:5px}.card strong{font-size:14px}.equipment{display:grid;grid-template-columns:46% 54%;gap:10px}.machine{height:210px;border:1px solid #d4a900;border-radius:7px;background:url('${d.photo}') center/cover no-repeat}.eqinfo{padding:14px 17px}.eqinfo h2{color:#ffd000;font-size:20px;margin:4px 0 14px}.eqinfo p{font-size:12px;line-height:1.55;min-height:54px}.specs{display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid #718087;padding-top:13px}.spec{padding:0 10px;border-right:1px solid #60737b}.spec:first-child{padding-left:0}.spec:last-child{border:0}.spec b{font-size:17px}.investment{display:grid;grid-template-columns:49% 51%;gap:10px}.price{min-height:145px;border:1px solid #e7ba00;border-left:10px solid #ffd000;border-radius:8px;padding:17px 18px 13px 72px;background:radial-gradient(circle at 30% 20%,#4b3a08 0,#101515 55%,#071014 100%);position:relative}.coins{position:absolute;left:20px;top:32px;color:#ffd000;font-size:42px}.price small{font-size:12px}.price strong{display:block;color:#ffd000;font-size:31px;margin:8px 0}.price span{font-size:12px;line-height:1.45}.conditions .card{min-height:44px;margin-bottom:5px;padding:7px 14px}.scope{padding:10px 14px;line-height:1.8;font-size:11px;position:relative;min-height:130px}.watermark{position:absolute;right:18px;bottom:0;font-size:80px;font-weight:900;color:rgba(191,145,28,.28)}.bottom{display:grid;grid-template-columns:1.25fr .75fr;gap:18px;align-items:center}.obs{background:#06171f;padding:10px 13px;font-size:10px;line-height:1.45}.quote{color:#ffd000;font-size:17px;font-weight:800;text-align:center}.footer{border-top:4px solid #e7ba00;background:linear-gradient(100deg,#061016 0,#061016 65%,#171206 100%);padding:15px 25px 18px}.contacts{display:flex;gap:28px;font-size:10px;align-items:center}.footbrand{text-align:right;color:#ffd000;font-size:24px;font-weight:900;margin-top:10px}.footbrand small{display:block;color:#fff;font-size:8px;font-weight:400}
 @media(max-width:760px){.hero{grid-template-columns:1fr 165px}.grid2,.grid3,.equipment,.investment,.bottom{grid-template-columns:1fr}.machine{height:230px}.price{padding-left:70px}}@media print{.toolbar{display:none}.stage{padding:0}.sheet{width:100%;border:0;box-shadow:none}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}@page{size:A4;margin:0}}
 </style></head><body><div class="toolbar"><b>Proposta Comercial — ${esc(num)}</b><div><button class="darkbtn" onclick="window.close()">Fechar</button><button class="goldbtn" onclick="window.print()">Imprimir / Salvar PDF</button></div></div><main class="stage"><article class="sheet">
 <header class="hero"><aside class="proposal"><h1>PROPOSTA <b>COMERCIAL</b></h1><div class="num">Nº: ${esc(num)}</div><div class="benefit">◈ SOLUÇÕES REAIS</div><div class="benefit">▰ MAIS PRODUTIVIDADE</div><div class="benefit">⚙ MENOR CUSTO OPERACIONAL</div><div class="benefit">◆ PARCERIA DE LONGO PRAZO</div><div class="benefit" style="margin-top:16px">DO PLANEJAMENTO À OPERAÇÃO, SEMPRE AO SEU LADO.</div></aside></header>
 <div class="content"><div class="section">● CLIENTE E PROPOSTA</div><div class="grid2"><div class="card"><small>Cliente</small><strong>${esc(p.client)}</strong></div><div class="card"><small>Empresa</small><strong>${esc(p.company||'—')}</strong></div></div><div class="grid3" style="margin-top:8px"><div class="card"><small>Modalidade</small><strong>${esc(p.mode)}</strong></div><div class="card"><small>Validade da proposta</small><strong>${esc(validity)}</strong></div><div class="card"><small>Data de emissão</small><strong>${esc(issue)}</strong></div></div>
 <div class="section">● EQUIPAMENTO</div><div class="equipment"><div class="machine"></div><div class="eqinfo"><h2>${esc(d.maker)} ${esc(d.model)} — ${esc(d.category)}</h2><p>${esc(d.desc)}</p><div class="specs">${d.specs.map(s=>`<div class="spec"><small>${esc(s[0])}</small><b>${esc(s[1])}</b></div>`).join('')}</div></div></div>
 <div class="section">● INVESTIMENTO</div><div class="investment"><div class="price"><div class="coins">▤</div><small>VALOR TOTAL</small><strong>${brl(p.value)}</strong><span>Investimento conforme proposta comercial<br>(Valores sujeitos à alteração)</span></div><div class="conditions"><div class="card"><small>Condições de pagamento</small><strong>${esc(p.terms||'A combinar / financiamento')}</strong></div><div class="card"><small>Prazo de entrega</small><strong>A combinar</strong></div><div class="card"><small>Garantia</small><strong>Conforme fabricante</strong></div></div></div>
 <div class="section">▤ ESCOPO DA PROPOSTA</div><div class="scope">✓ ${esc(scope)}<br>✓ Suporte na análise técnica e comparativa com outras opções de mercado.<br>✓ Orientação sobre manutenção preventiva e custo operacional.<br>✓ Acompanhamento no processo de aquisição e entrega do equipamento.<br>✓ Treinamento operacional básico (conforme fabricante).<span class="watermark">M</span></div>
 ...
 <footer class="footer"><div class="contacts"><span>☎ (98) 99999-9999</span><span>✉ contato@maqon.com.br</span><span>in /maqon</span></div><div class="footbrand">MAQON<small>CONSULTORIA EM EQUIPAMENTOS PESADOS</small></div></footer>
